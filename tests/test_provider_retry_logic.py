...
def _mock_chat_response(content: str = "retry success") -> SimpleNamespace:
    ...
    return SimpleNamespace(choices=[choice], model="gpt-5.3-chat-latest", id="resp-1", created=123, usage=usage)
...
def test_openai_provider_retries_on_transient_error(monkeypatch):
    ...
    result = provider.generate_content("hello", "gpt-5.3-chat-latest")
...
def test_openai_provider_bails_on_non_retryable_error(monkeypatch):
    ...
    with pytest.raises(RuntimeError) as excinfo:
        provider.generate_content("hello", "gpt-5.3-chat-latest")
