...
class TestIntelligentFallback:
    ...
    def test_prefers_openai_o3_mini_when_available(self):
        """Test that gpt-5 is preferred when OpenAI API key is available (based on current catalog)"""
        ...
        assert fallback_model == "gpt-5"  # Flagship gpt-5 (alias of gpt-5.5); gpt-5.2 no longer in catalog
    ...
    def test_prefers_gemini_flash_when_openai_unavailable(self):
        """Test that the current flash model is used when only Gemini API key is available"""
        ...
        fallback_model = ModelProviderRegistry.get_preferred_fallback_model()
        assert fallback_model == "gemini-3-flash-preview"
    ...
    def test_prefers_openai_when_both_available(self):
        ...
        fallback_model = ModelProviderRegistry.get_preferred_fallback_model()
        assert fallback_model == "gemini-3-flash-preview"  # Gemini has priority now (based on new PROVIDER_PRIORITY_ORDER)
    ...
    def test_auto_mode_conversation_memory_integration(self):
        ...
        with (
            ...
            with patch("utils.model_context.ModelContext") as mock_context_class:
                ...
                # Verify that ModelContext was called with gpt-5 (the intelligent fallback based on current catalog)
    ...
    def test_auto_mode_with_gemini_only(self):
        ...
        with (
            ...
            with patch("utils.model_context.ModelContext") as mock_context_class:
                ...
                # Should use the current flash model when only Gemini is available
                mock_context_class.assert_called_once_with("gemini-3-flash-preview")
